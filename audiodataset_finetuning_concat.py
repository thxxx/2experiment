...
class AudioDataset(Dataset):
    ...
    def pre_process(self, audio_path, total_duration):
        ...
        recorded_length = wav.signal_length
        ...
        return wav.audio_data, length, recorded_length
    ...
    def get_concat_audio(self, wav1, wav2):
        return 
    ...
    def __getitem__(self, idx):
        ...
        wav, length, resampled_length = self.pre_process(audio_path, total_duration)
        wav = wav.squeeze(1)

        if self.mixed:
            if data['added_audio_path'] != None and isinstance(data['added_audio_path'], str):
                # mix
                new_wav, new_length, new_resampled_length = self.pre_process(data['added_audio_path'], 1.5)
                # print("new_length : ", new_length, new_wav.shape)
                new_wav = new_wav.squeeze(1)
                wav = wav[:,:resampled_length]
                new_wav = new_wav[:,:new_resampled_length]
                normalized_wav = self.normalize(wav)
                normalized_new_wav = self.normalize(new_wav)
                pad = torch.zeros((1, int(self.duration * self.target_sample_rate) - resampled_length - new_resampled_length))
